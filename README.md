...
首次运行、状态文件被删除、损坏或版本不受支持时，服务会扫描全部分页并从已有帖子目录重建状态。状态有效后，每轮从最新分页开始，只在连续两个分页全部是已记录帖子后停止。置顶或取消置顶不会导致已记录帖子重复下载。

服务不会按固定时间自动执行完整扫描。需要检查快速扫描范围之外的历史解锁或回填帖子时，删除 `.fanbox-state.json` 后重新运行；已有帖子目录会被重新识别，不会全部重下。
