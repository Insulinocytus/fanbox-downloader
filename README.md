...
`.fanbox-state.json` 按作者和帖子 ID 记录已下载、无文件或当前受限的帖子。容器重建后只要重新挂载同一下载目录，就会继续使用该状态；不需要额外的状态 volume。

首次运行、状态文件被删除、损坏或版本不受支持时，服务会扫描全部分页，并通过正常帖子处理结果重建状态；已有帖子目录本身不会被直接视为已下载。状态有效后，每轮从最新分页开始，只在连续两个分页全部是已记录帖子后停止。置顶或取消置顶不会导致已记录帖子重复下载。

服务不会按固定时间自动执行完整扫描。需要检查快速扫描范围之外的历史解锁或回填帖子时，删除 `.fanbox-state.json` 后重新运行；全部历史帖子会被重新检查，但已有文件仍会被跳过，不会重复下载。
